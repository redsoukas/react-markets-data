...
export const CoinDetails = () => {
  ...
  return (
    <div className='container'>
      <button className="tag" onClick={() => {navigate('/')}}>All coins</button>
      <div>
        ...
        <p>
          The highest price since the creation of the coin (ATH) was <strong>{coinData?.ath}</strong> at <strong>{coinData?.athDate}</strong> & the lowest price since the creation of the coin (ATL) was <strong>{coinData?.atl}</strong> at <strong>{coinData?.atlDate}</strong>
        </p>
      ...
      <div className={styles.chartFilters}>
        ...
        <select value={chartInterval} onChange={(e) => {setChartInterval(e.target.value)}} id="interval">
          {
            Object.keys(chartIntervals).map( (key: any) => {
              return (
                <option key={key}>{chartIntervals[key]}</option>
              )
            })
          }
        </select>
      ...
      <label>social</label>
      {
        Object.keys(social).map(
          key => {
            ...
            return (
              <li key={key}>
                <a className="tag" target="_blank" rel="noopener noreferrer" href={link}>{key}</a>
...
export const PriceChange = ( {priceChangePercentage}: Percentage) => {
  return (
    <ul className={styles.info}>
      ...
      {
        Object.keys(priceChangePercentage)?.map(
          key => {
            ...
            return (
              <li key={contact[key]}>
                ...
                { contact[key]?.map( (link: string) => {
                  if (!link) return null;
                  return <a className="tag" target="_blank" rel="noreferrer noopener" key={link} href={link}>{link}</a>;
                })
                }
              </li>
            );
    ...
    <div>
      <ul className={styles.info}>
        ...
        <li>
          {repos?.map( repo => {
            return (
              <a key={repo} className="tag" target="_blank" rel="noreferrer noopener" href={repo}>{repo}</a>
            );
          })}
        </li>
        {forks && <li><span>Forks: {forks}</span></li>}
        {stars &&  <li><span>Stars: {stars}</span></li>}
        {subscribers && <li><span>Subscribers: {subscribers}</span></li>}
        {issues && <li><span>Issues: {issues}</span></li>}
      </ul>
    </div>
  );
}