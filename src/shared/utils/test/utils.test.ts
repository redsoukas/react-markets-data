  ...
  describe('setUrlParams tests', () => {
    it('Should set params properly to url without initial query params', () => {
      ...
      expect(urlWithParams.href).toEqual(
        `https://api.coingecko.com/api/v3/coins/markets?page=${paramsObject.page}&vs_currency=${paramsObject.vs_currency}`
      );
    });
    ...
    it('Should set params properly to url with initial query params', () => {
      ...
      const urlWithParams = setUrlParams(url, paramsObject);
      expect(urlWithParams.href).toEqual(
        `https://api.coingecko.com/api/v3/coins/markets?page=${paramsObject.page}&vs_currency=${paramsObject.vs_currency}&per_page=${paramsObject.per_page}`
      );
    });
  });
});
